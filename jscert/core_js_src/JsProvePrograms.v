...
Lemma always_access_field_normal :
  forall h L e h' r,
  red h L e h' (result_ref r) ->
  exists n l, r = Ref l (field_normal n).
Proof.
introv R.
sets_eq v E: (result_ref r).
(*
...
assert (AllBefore (fun l => ~in_protochain (write h l' f v) f l l') l' L) as H.
  ...
  constructor.
    intro D'; subst.
    apply scopes_result_has_proto in S; auto.
    lets* (l'0 & H1 & H2): S.
  ...
  assert (scopes h f (a :: L) a) as Sa.
    assert (exists x, proto h f l' x /\ x <> loc_null) as [x [H1 H2]].
      applys* scopes_result_has_proto L.
      introv I; apply B.
      right~.
  ...
  forwards*: scopes_result_has_proto h f L l'.
...
assert (AllBefore (fun l => proto h f l loc_null /\ ~in_protochain h f l l') l L) as AB.
  ...
  intro IPC.
  assert (scopes h f (a :: L) a) as Sa.
    forwards* (r & ? & ?): scopes_result_has_proto h f (a :: L) l.
...
introv WF DefLoc.
eexists.
exists value_undef.  (* Here we know the result of the program, but that may
...
Existential 1 := expr_variable "". Existential 1 := result_value (value_loc loc_null).
Existential 1 := result_value (value_loc loc_null).
